...
if (isset($_POST['item_ids_output'])) {
    ...
    if ($res_id_schedule && mysqli_num_rows($res_id_schedule) > 0) {
        $row = mysqli_fetch_assoc($res_id_schedule);
        $latest_id_schedule = $row['id_schedule'];
    }else{
        $latest_id_schedule = 0;
    }
                                ...
                                if ($count == 1 ){
                                    ...
                                    if (!in_array($truck_id, $updated_trucks)) {
                                        ...
                                        if (mysqli_num_rows($res_check_truck_schedule) == 0) {
                                            $sql_update_truck = "UPDATE `truck` SET `truck_status` = 3, `id_location` = ".$row['id_location_from']." WHERE `id` = '".($trucks_ids[$truck_id])."'";
                                            ...
                                            $sql_insert_truck_driver = "INSERT INTO `truck_driver` (`id`,`id_truck`,`id_driver1`, `id_driver2`) VALUES (".$schedule_id_counter.", ".($trucks_ids[$truck_id]).", 1, 2);";
